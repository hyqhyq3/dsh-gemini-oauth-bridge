...
const DEFAULT_MODELS = ['gemini-3-pro', 'gemini-3-flash', 'gemini-2.5-pro', 'gemini-2.5-flash'];
...
function loadState() {
  try {
    ...
    return { tokens: null, email: null, project: null, tier: null, models: null, antigravityVersion: null, versionFetchedAt: 0, signatures: {}, apiKey: null, apiConfig: {} };
  }
}
...
function defaultTierId(loadResp) {
  const tiers = Array.isArray(loadResp?.allowedTiers) ? loadResp.allowedTiers : [];
  for (const tier of tiers) {
    if (tier && typeof tier === 'object' && tier.isDefault === true && typeof tier.id === 'string' && tier.id.trim()) {
      return tier.id.trim();
    }
  }
  if (typeof loadResp?.currentTier?.id === 'string' && loadResp.currentTier.id.trim()) return loadResp.currentTier.id.trim();
  return 'free-tier';
}
...
async function fetchAvailableModels(token) {
  const ua = userAgents(currentVersion());
  const res = await upstreamJson(`${baseURLs().generate}${API.fetchModelsPath}`, {
    token,
    ua: ua.short,
    body: { metadata: { ideType: 'ANTIGRAVITY' } },
    timeoutMs: 20000,
  });
  if (res.status !== 200) throw new Error(`fetchAvailableModels failed (${res.status}): ${res.text.slice(0, 200)}`);
  const list = Array.isArray(res.json?.models) ? res.json.models : null;
  if (!list) throw new Error('fetchAvailableModels returned no models array');
  const ids = list
    .map((entry) => (typeof entry === 'string' ? entry : entry && typeof entry === 'object' ? entry.id || entry.name || entry.modelId : null))
    .filter((id) => typeof id === 'string' && id);
  if (!ids.length) throw new Error('fetchAvailableModels returned an empty model list');
  return ids;
}
      ...
      try {
        ...
        if (path === '/gemini-oauth-bridge/api/refresh-models' && req.method === 'POST') {
          ...
          try {
            ...
            saveState();
            return json(res, 200, { ok: true, models: state.models });
          } catch (error) {
